...
class SignInBloc extends Bloc<SignInEvent, SignInState> {
  ...
  Future<void> _onClinical(
    ...
    emit(
      state.copyWith(status: SignInStatus.loading, selectedType: "clinical"),
    );
    ...
    try {
      ...
      emit(
        state.copyWith(status: SignInStatus.success, selectedType: "clinical"),
      );
    } catch (e) {
      emit(
        state.copyWith(
  ...
  Future<void> _onCorporate(
    ...
    emit(
      state.copyWith(status: SignInStatus.loading, selectedType: "corporate"),
    );

    try {
      await Future.delayed(const Duration(milliseconds: 300)); // placeholder
      emit(
        state.copyWith(status: SignInStatus.success, selectedType: "corporate"),
      );
    } catch (e) {
      emit(
        state.copyWith(
